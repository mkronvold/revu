...
describe('assessment authoring and review API', () => {
  ...
  it('moves an employee assessment set through accepted, ready, scheduled, and reviewer conclusions', async () => {
    ...
    const createdPeerAssessment = assessmentItemResponseSchema.parse(
      createPeerResponse.json(),
    ).item;
    ...
    expect(assessmentItemResponseSchema.parse(saveDraftResponse.json()).item.reviewState).toBe(
      'draft',
    );
    ...
    expect(assessmentItemResponseSchema.parse(submitResponse.json()).item.reviewState).toBe(
      'submitted',
    );
    ...
    expect(assessmentItemResponseSchema.parse(acceptSelfResponse.json()).item.reviewState).toBe(
      'accepted',
    );
    ...
    expect(assessmentItemResponseSchema.parse(acceptPeerResponse.json()).item.reviewState).toBe(
      'accepted',
    );
    ...
    expect(
      assessmentsListResponseSchema
        .parse(patVisibleAcceptedResponse.json())
        .items.filter((item) => item.employeeId === pat.user.id)
        .map((item) => item.id)
        .sort(),
    ...
    expect(
      assessmentsListResponseSchema
        .parse(adaVisibleAcceptedResponse.json())
        .items.filter((item) => item.employeeId === pat.user.id)
        .map((item) => item.id)
        .sort(),
    ).toEqual([createdPeerAssessment.id, createdSelfAssessment.id].sort());
    ...
    expect(readyAssessmentSet.items.every((item) => item.reviewState === 'ready_for_meeting')).toBe(
      true,
    );
    ...
    expect(scheduledAssessmentSet.items.every((item) => item.reviewState === 'scheduled')).toBe(
      true,
    );
    expect(
      scheduledAssessmentSet.items.every((item) => item.scheduledByEmployeeId === manny.user.id),
    ).toBe(true);
    ...
    const reviewer1CompletedSet = assessmentSetResponseSchema.parse(
      reviewer1CompleteResponse.json(),
    );
    expect(reviewer1CompletedSet.items.every((item) => item.reviewState === 'scheduled')).toBe(
      true,
    );
    expect(
      reviewer1CompletedSet.items.every(
        (item) => item.reviewer1CompletedByEmployeeId === ada.user.id,
      ),
    ).toBe(true);
    expect(
      reviewer1CompletedSet.items.every((item) => item.reviewer2CompletedByEmployeeId === null),
    ).toBe(true);
    ...
    const concludedAssessmentSet = assessmentSetResponseSchema.parse(
      reviewer2CompleteResponse.json(),
    );
    expect(concludedAssessmentSet.items.every((item) => item.reviewState === 'concluded')).toBe(
      true,
    );
    expect(
      concludedAssessmentSet.items.every((item) => item.concludedByEmployeeId === manny.user.id),
    ).toBe(true);
    ...
    expect(reopenedAssessmentSet.items.every((item) => item.reviewState === 'scheduled')).toBe(
      true,
    );
    expect(
      reopenedAssessmentSet.items.every((item) => item.reviewer1CompletedByEmployeeId === null),
    ).toBe(true);
    expect(
      reopenedAssessmentSet.items.every(
        (item) => item.reviewer2CompletedByEmployeeId === manny.user.id,
      ),
    ).toBe(true);
    expect(reopenedAssessmentSet.items.every((item) => item.concludedByEmployeeId === null)).toBe(
      true,
    );
  });
  ...
  it('supports peer rejection back to draft, reassignment, and role-aware assessment visibility', async () => {
    ...
    const elliotVisibleAssessments = assessmentsListResponseSchema.parse(
      elliotVisibleAssessmentsResponse.json(),
    ).items;
    expect(elliotVisibleAssessments.some((item) => item.id === assessment.id)).toBe(true);
    expect(
      elliotVisibleAssessments.some((item) => item.id === 'dddddddd-dddd-4ddd-8ddd-dddddddddddd'),
    ).toBe(true);
    expect(
      elliotVisibleAssessments.some((item) => item.id === 'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee'),
    ).toBe(true);
    ...
    const filteredManagerAssessments = assessmentsListResponseSchema.parse(
      filteredManagerAssessmentsResponse.json(),
    ).items;
  ...
  it('lets admins override assessment responses, notes, state, and deletion', async () => {
    ...
    const acceptedAssessment = assessmentItemResponseSchema.parse(acceptResponse.json()).item;
    expect(acceptedAssessment.reviewState).toBe('accepted');
    expect(acceptedAssessment.managerNotes).toBe(
      'Admin override acceptance for follow-up testing.',
    );
    ...
    expect(
      assessmentsListResponseSchema.parse(listResponse.json()).items.map((item) => item.id),
    ).not.toContain(createdAssessment.id);
  });
  ...
  it('enforces archive-aware read-only behavior for assessment creation and updates', async () => {
    ...
    expect(
      assessmentItemResponseSchema.parse(archivedAssessmentResponse.json()).item.archiveState,
    ).toBe('archived');
  ...
  it('captures assessment workflow protections in SQL', () => {
    const sql = readFileSync(
      resolve(process.cwd(), '../../prisma/migrations/004_assessment_review_api.sql'),
      'utf8',
    );
