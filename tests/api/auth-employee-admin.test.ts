...
describe('auth and employee admin API', () => {
  ...
  it('lets an authenticated user update their own profile', async () => {
    const app = await createApp();
    const employeeLogin = authLoginResponseSchema.parse(
      (await login(app, 'elliot.employee', 'EmployeePass123!')).json(),
    );
    ...
    expect(meResponse.statusCode).toBe(200);
    expect(authMeResponseSchema.parse(meResponse.json()).session.user.fullName).toBe(
      'Elliot Updated',
    );
  });
  ...
  it('enforces RBAC while supporting employee CRUD and admin password flows', async () => {
    ...
    const managerLogin = authLoginResponseSchema.parse(
      (await login(app, 'manny.manager', 'ManagerPass123!')).json(),
    );
    ...
    expect(
      employeesListResponseSchema.parse(listResponse.json()).items.length,
    ...
    const adminLogin = authLoginResponseSchema.parse(
      (await login(app, 'ada.admin', 'AdminPass123!')).json(),
    );
    ...
    expect(
      setEmployeePasswordResponseSchema.parse(setPasswordResponse.json()).passwordResetRequired,
    ).toBe(false);
    ...
    const resetLoginPayload = authLoginResponseSchema.parse(
      (await login(app, 'new.hire', resetPayload.temporaryPassword)).json(),
    );
    ...
    expect(authMeResponseSchema.parse(resetMeResponse.json()).session.passwordResetRequired).toBe(
      true,
    );
    ...
    const changedPasswordPayload = authChangePasswordResponseSchema.parse(
    ...
    expect(
      listedEmployeesAfterDelete.items.some((employee) => employee.id === createdEmployee.id),
    ).toBe(false);
    ...
    expect(employeeResponseSchema.parse(recreateDeletedEmployeeResponse.json()).item.username).toBe(
      'New.Hire',
    );
  });
  ...
  it('supports local user import/export with password transfer fields', async () => {
    ...
    const exportAdminLogin = authLoginResponseSchema.parse(
      (await login(exportApp, 'ada.admin', 'AdminPass123!')).json(),
    );
    ...
    const exportedEmployee = exportPayload.items.find(
      (item) => item.username === 'elliot.employee',
    );
    ...
    const exportedAdminLogin = authLoginResponseSchema.parse(
      (await login(exportApp, 'ada.admin', exportedAdmin!.password)).json(),
    );
    expect(exportedAdminLogin.session.passwordResetRequired).toBe(true);

    const importApp = await createApp();
    const importAdminLogin = authLoginResponseSchema.parse(
      (await login(importApp, 'ada.admin', 'AdminPass123!')).json(),
    );
    ...
    expect(
      importPayload.items.find((item) => item.username === 'elliot.employee')?.auth
        .passwordResetRequired,
    ).toBe(true);
    expect(
      importPayload.items.find((item) => item.username === 'new.transfer')?.auth
        .passwordResetRequired,
    ).toBe(false);

    expect((await login(importApp, 'elliot.employee', 'EmployeePass123!')).statusCode).toBe(401);
    const importedEmployeeLogin = authLoginResponseSchema.parse(
      (await login(importApp, 'elliot.employee', 'ImportedPass123!')).json(),
    );
    expect(importedEmployeeLogin.session.passwordResetRequired).toBe(true);

    const transferredUserLogin = authLoginResponseSchema.parse(
      (await login(importApp, 'new.transfer', 'TransferPass123!')).json(),
    );
  ...
  it("removes deleted employees' subject assessments while keeping completed peer assessments on the tombstone", async () => {
    ...
    const managerLogin = authLoginResponseSchema.parse(
      (await login(app, 'former.manager', 'FormerManager123!')).json(),
    );
    ...
    const completedAssignment = assignmentResponseSchema.parse(
      completedAssignmentResponse.json(),
    ).item;
    ...
    const completedPeerAssessment = assessmentItemResponseSchema.parse(
      completedPeerAssessmentResponse.json(),
    ).item;
    ...
    const draftPeerAssessment = assessmentItemResponseSchema.parse(
      draftPeerAssessmentResponse.json(),
    ).item;
    ...
    expect(employeeResponseSchema.parse(employeeAfterDeleteResponse.json()).item.managerId).toBe(
      deletedManager.id,
    );
    ...
    expect(
      foundation.assignments.find(
        (assignment) => assignment.id === 'cccccccc-cccc-4ccc-8ccc-cccccccccccc',
      )?.managerId,
    ).toBe(deletedManager.id);
    expect(foundation.assessments.some((assessment) => assessment.id === selfAssessment.id)).toBe(
      false,
    );
    expect(
      foundation.assessments.some((assessment) => assessment.id === draftPeerAssessment.id),
    ).toBe(false);
    expect(
      foundation.assessments.find((assessment) => assessment.id === completedPeerAssessment.id),
    ).toMatchObject({
      employeeId: '44444444-4444-4444-8444-444444444444',
      assessorId: deletedManager.id,
  ...
  it('rejects imported password-hash credentials that do not use a supported stored-password format', async () => {
    ...
    expect(importResponse.body).toContain(
      'Password hash must use a supported stored-password format',
    );
  });
  ...
  it('captures auth schema invariants in the migration', () => {
    const authSql = readFileSync(
      resolve(process.cwd(), '../../prisma/migrations/002_auth_employee_admin.sql'),
      'utf8',
    );
    ...
    const tombstoneSql = readFileSync(
      resolve(process.cwd(), '../../prisma/migrations/007_employee_tombstones.sql'),
      'utf8',
    );
