...
describe('API/domain foundation', () => {
  ...
  it('serves typed foundation resources', async () => {
    ...
    expect(
      domainRulesResponseSchema.parse(rulesResponse.json()).acceptedAssessmentsAreImmutable,
    ).toBe(false);
    expect(employeesResponse.statusCode).toBe(200);
    expect(
      employeesListResponseSchema.parse(employeesResponse.json()).items.length,
    ).toBeGreaterThan(0);
    ...
    expect(activeQuestionSets.filter((questionSet) => questionSet.target === 'self')).toHaveLength(
      1,
    );
    expect(activeQuestionSets.filter((questionSet) => questionSet.target === 'peer')).toHaveLength(
      1,
    );
