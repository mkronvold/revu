...
describe('review periods, question sets, and assignments admin API', () => {
  ...
  it('supports admin CRUD flows for review periods, question sets, assignment sync, and question-set export downloads', async () => {
    ...
    const createdReviewPeriod = reviewPeriodResponseSchema.parse(
      createdReviewPeriodResponse.json(),
    ).item;
    ...
    expect(reviewPeriodResponseSchema.parse(updatedReviewPeriodResponse.json()).item.label).toBe(
      '2027 Performance Review',
    );
    ...
    expect(
      reviewPeriodResponseSchema.parse(relaxedDeadlineUpdateResponse.json()).item,
    ).toMatchObject({
      startDate: '2027-01-10',
      dueDate: '2027-01-20',
      assessmentDueDate: '2027-01-05',
    ...
    const firstSelfQuestionSet = questionSetResponseSchema.parse(
      firstSelfQuestionSetResponse.json(),
    ).item;
    ...
    const secondSelfQuestionSet = questionSetResponseSchema.parse(
      secondSelfQuestionSetResponse.json(),
    ).item;
    ...
    expect(questionSetResponseSchema.parse(activateFirstResponse.json()).item.status).toBe(
      'active',
    );
    ...
    const filteredQuestionSets = questionSetsListResponseSchema.parse(
      filteredQuestionSetsResponse.json(),
    ).items;
    expect(
      filteredQuestionSets.filter((item) => item.target === 'self' && item.status === 'active'),
    ).toHaveLength(1);
    expect(filteredQuestionSets.find((item) => item.id === firstSelfQuestionSet.id)?.status).toBe(
      'draft',
    );
    expect(filteredQuestionSets.find((item) => item.id === secondSelfQuestionSet.id)?.status).toBe(
      'active',
    );
    ...
    const employeeAfterAssignment = employeeResponseSchema.parse(
      employeeAfterAssignmentResponse.json(),
    ).item;
    ...
    const questionSetExport = questionSetsExportResponseSchema.parse(
      questionSetExportResponse.json(),
    );
    expect(questionSetExport.itemCount).toBe(2);
    expect(questionSetExport.items.map((item) => item.title)).toEqual([
      '2027 Self Questions v1',
      '2027 Self Questions v2',
    ]);
    ...
    const questionSetCsvExport = questionSetsExportResponseSchema.parse(
      questionSetCsvExportResponse.json(),
    );
    ...
    const questionSetImport = questionSetsImportResponseSchema.parse(
      questionSetImportResponse.json(),
    );
  ...
  it('preserves referenced question ids on update and blocks removing questions with recorded responses', async () => {
    ...
    expect(updatedQuestionSet.questions.map((question) => question.id)).toEqual(
    ...
    expect(removeReferencedQuestionResponse.body).toContain(
      'cannot be removed from a question set',
    );
  });
  ...
  it('reorders peer question sets safely after removing an unreferenced question', async () => {
    ...
    const createdReviewPeriod = reviewPeriodResponseSchema.parse(
      createReviewPeriodResponse.json(),
    ).item;
    ...
    const createdQuestionSet = questionSetResponseSchema.parse(
      createQuestionSetResponse.json(),
    ).item;
    ...
    expect(updatedQuestionSet.questions.map((question) => question.prompt)).toEqual([
      'Question 1',
      'Question 3 Updated',
    ]);
  });
  ...
  it('deletes a review period together with its question sets, assessments, and assignments', async () => {
    const app = await createApp();
    const session = await loginAsAdmin(app);
    const reviewPeriodToDelete =
      foundationSnapshotExample.reviewPeriods.find(
        (reviewPeriod) => reviewPeriod.status === 'archived',
    ...
    expect(
      questionSetsListResponseSchema.parse(remainingQuestionSetsResponse.json()).items,
    ).toHaveLength(0);
    ...
    expect(
      assessmentsListResponseSchema.parse(remainingAssessmentsResponse.json()).items,
    ).toHaveLength(0);
  ...
  it('clears ready-to-start assessments for the active review period', async () => {
    ...
    const assessmentsBeforeClear = assessmentsListResponseSchema.parse(
      assessmentsBeforeClearResponse.json(),
    ).items;
    ...
    expect(
      assessmentsListResponseSchema.parse(assessmentsAfterClearResponse.json()).items,
    ).toHaveLength(0);
  });
  ...
  it('syncs only active employee-assessor pairs and removes stale not-started assessments', async () => {
    ...
    const listAssessmentKeys = async () => {
      ...
      expect(response.statusCode).toBe(200);
      return assessmentsListResponseSchema
        .parse(response.json())
        .items.map((assessment) => `${assessment.employeeId}:${assessment.assessorId}`);
    };
  ...
  it('archives and unarchives review periods while enforcing archive-only mutations', async () => {
    ...
    const archivedQuestionSets = questionSetsListResponseSchema.parse(
      archivedQuestionSetsResponse.json(),
    ).items;
    ...
    expect(
      archivedAssessments.every(
        (assessment) => assessment.archiveState === 'archived' && assessment.isReadOnly,
      ),
    ).toBe(true);
    ...
    expect(
      unarchivedFoundation.questionSets
        .filter(
          (questionSet) => questionSet.reviewPeriodId === 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
        )
  ...
  it('captures review-period archive sync and assignment guard rails in SQL', () => {
    const sql = readFileSync(
      new URL('../../prisma/migrations/003_review_periods_assignments_api.sql', import.meta.url),
      'utf8',
    );
